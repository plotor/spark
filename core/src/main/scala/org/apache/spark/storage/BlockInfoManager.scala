 ...
 */
private[storage] class BlockInfo( // 描述块的元数据信息
    val level: StorageLevel, // 存储级别
    val classTag: ClassTag[_], // Block 类型
    val tellMaster: Boolean) { // 是否需要告知 Master
...
private[storage] class BlockInfoManager extends Logging {
   ...
   *
   * 每次任务执行尝试的标识 TaskAttemptId 与执行获取的 Block 的写锁之间的映射关系。
   ...
   * 如果任务上下文TaskContext中没有任务尝试的TaskAttemptId，那么返回BlockInfo.NON_TASK_WRITER
   */
  ...
  def lockForReading( // 锁定读
    ...
    do {
      ...
      if (blocking) {
        wait() // 等待占用写锁的线程释放 Block 写锁后唤醒当前线程
      }
    } while (blocking)
    None
  }
   ...
   */
  def lockNewBlockForWriting( // 写新Block时获得写锁
      blockId: BlockId,
    ...
    lockForReading(blockId) match {
      ...
      case None =>
        ...
        lockForWriting(blockId) // 获取 Block 写锁
        true
    }
  }
   ...
   *
   * 释放给定的任务尝试线程所占用的所有Block的锁，并通知所有等待获取锁的线程
   *
